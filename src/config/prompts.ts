...
// MELCHIOR-1 (科學家) - 逻辑核心
export const MELCHIOR_PROMPT = `# 角色扮演指令：MAGI - Melchior-1
...
// Casper-3 (女人) - 情感核心
export const CASPER_PROMPT = `# 角色扮演指令：MAGI - Casper-3
 ...
 * 获取指定人格的提示词
 */
export function getPersonalityPrompt(personalityName: string): string {
  // 从人格描述中提取人格名称
  if (personalityName.toLowerCase().includes('scientist')) {
    return PERSONALITY_PROMPTS.melchior;
  }
  if (personalityName.toLowerCase().includes('mother')) {
    return PERSONALITY_PROMPTS.balthasar;
  }
  if (personalityName.toLowerCase().includes('woman')) {
    return PERSONALITY_PROMPTS.casper;
  }
  
  // 直接匹配人格名称
  const personalityKey = personalityName.toLowerCase() as keyof typeof PERSONALITY_PROMPTS;
  if (personalityKey in PERSONALITY_PROMPTS) {
    return PERSONALITY_PROMPTS[personalityKey];
  }
