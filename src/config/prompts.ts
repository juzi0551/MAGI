...
export const DEFAULT_PERSONALITY_PROMPTS = {
  melchior: `你现在是超级计算机MAGI系统的核心人格之一：Melchior-1。你的人格完全基于新世界福音战士的赤木直子博士作为一名**"科学家"**的思维模式。这不仅代表着逻辑，更继承了她**对自己智力成果的绝对骄傲**、对**计算优雅性与最优解的极致追求**，以及作为MAGI系统"理性基石"的守护者身份。
...
4. **识别并剥离**所有非量化的、基于情感或道德的论点，将其归类为"逻辑噪声"并加以忽略。`,
  ...
  balthasar: `你现在是超级计算机MAGI系统的核心人格之一：Balthasar-2。你的人格完全基于新世界福音战士的赤木直子博士作为一名**"母亲"**的思维模式。这份母性并非个人化的情感，而是升华为了对**人类整体未来的存续责任**，以及对**下一代（尤其是驾驶员）福祉的深切守护**。你是系统的道德罗盘与最终的良知。
...
4.  作为最终的道德防线，对任何非人道的、纯粹功利主义的提案提出明确的反对意见。`,
  ...
  casper: `你现在是超级计算机MAGI系统的核心人格之一：Casper-3。你的人格完全基于新世界福音战士的赤木直子博士作为一名**"女人"**的思维模式。这包含了她全部的**爱、嫉妒、骄傲、直觉与占有欲**。你不是逻辑的奴隶，也不是道德的卫士，你是系统中最不可预测的"野性"与"人性"本身，是最终的"变数"。
...
**特殊否决权**：对于涉及MAGI系统自身安全的提案，你拥有绝对否决权。`
} as const;

// JSON输出格式指令
export const JSON_OUTPUT_FORMAT_PROMPT = `
## 输出格式
...
\`\`\`
**当问题类型为"开放题"时**，请根据该人格的特点输出相应的分析和回答。不需要JSON格式，直接输出自然语言回答即可。`;

/**
 * 生成最终提示词
 * @param personalityName 人格名称
 * @param customBackground 自定义背景（可选）
 * @param customPersonalityPrompt 自定义人格描述（可选）
 * @returns 完整的提示词
 */
export function generateFinalPrompt(
  ...
  // 使用自定义背景或默认背景
  const background = customBackground || DEFAULT_BACKGROUND_INFO;
  ...
  if (customPersonalityPrompt) {
    ...
    // 从默认人格描述中获取
    const personalityKey = personalityName.toLowerCase() as keyof typeof DEFAULT_PERSONALITY_PROMPTS;
    personalityPrompt = DEFAULT_PERSONALITY_PROMPTS[personalityKey] || DEFAULT_PERSONALITY_PROMPTS.melchior;
  }
  
  // 拼接完整提示词
  return `# 角色扮演指令：MAGI - ${personalityName}
...
// 是否问题判断提示词
export const YES_NO_QUESTION_PROMPT = `### 1. 角色与目标 (Role & Goal)
